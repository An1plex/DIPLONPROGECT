...
{
    // Сначала обработка ошибок
    // ----------------------------------------------------------------------------------------------------   
    var error = false;
    // Если селектор не задан
    if(!params.selector) {                                              console.error("селектор должен быть определен"); error = true; }
    // If the selector is not a string  Если селектор не является строкой
    if(typeof params.selector != "string") {                            console.error("селектор должен быть строкой"); error = true; }
    // If the element is not a form  Если элемент не является формой
    if(!$(params.selector).is("form")) {                                console.error("Element needs to be a form"); error = true; }
    // If the element doesn't contain a select Если элемент не содержит выбора
    if($(params.selector).find("select").length < 1) {                  console.error("Element needs to have a select in it"); error = true; }
    // If the element doesn't contain option elements Если элемент не содержит элементов опций
    if($(params.selector).find("option").length < 1) {                  console.error("Select element needs to have an option in it"); error = true; }
    // If the select element doesn't have a name attribute Если выбранный элемент не имеет атрибута name
    ...
    // If there was an error at all, dont continue in the code. Если вообще произошла ошибка, не продолжайте в коде.
    if(error)
        return false;
    // ----------------------------------------------------------------------------------------------------
    var that            = this,
        ...
        currently_selected      = [],     
        // Create the elements needed for the checkbox select Создайте элементы, необходимые для выбора флажка
        ...
        // Template for the li, will be used in a loop. Шаблон для li, будет использоваться в цикле.
		...
		apply = function()
		{
			...
			{  
				// Only do the Apply event if its different Выполняйте событие Apply только в том случае, если оно отличается
    ...
    // Event of this instance Событие этого экземпляра
    that.onApply = typeof params.onApply == "function" ?             
                    params.onApply :
                    function(e) 
                    {
                        //e.selected is accessible доступный
                    };
    ...
    that.check = function(checkbox_name, checked) 
    {
		...
		{
			// If this elements name is equal to the one sent in the function Если имя этого элемента равно имени, отправленному в функции
			if($(this).attr('name') == checkbox_name)
			{
				// Apply the checked state to this checkbox Примените отмеченное состояние к этому флажку
				$(this).attr('checked', checked ? checked : false);				
				// Break out of each loop Вырваться из каждого цикла
				return false;
			}
		});
        updateCurrentlySelected();
    }
    // Build mark up before pushing into page  Создайте разметку перед вводом на страницу
    ...
    // Iterate through option elements  Перебирать элементы опций
    that.update();
    // Events   События
    // Actual dropdown action   Фактическое действие выпадающего списка
    ...
    // Filters the checkboxes by search on keyup Фильтрует флажки по поиску в keyup
    $_search.keyup(
        function()
        {
    ...
    // Delete the original form submit Фильтрует флажки с помощью поиска по keyupDelete исходной формы отправки
    $(params.selector).find('input[type=submit]').remove();
    // Put finalized markup into page. Поместите окончательную разметку на страницу.
    $_native_select.after($_parent_div);
    // Hide the original element Скрыть исходный элемент
    $_native_select.hide();
};